...
class Pokemon:
    ...
    def from_memory_buffer(cls, nickname: str, caught_by: str, buffer) -> Self:
        return Pokemon(
            ...
            move1_max_pp=MOVE_BASE_PP[GEN_1_MOVES[buffer[0x8]]] + ((int(buffer[0x1D]) >> 6) & 0x03),
            move2=GEN_1_MOVES[buffer[0x9]],
            move2_pp=(int(buffer[0x1E]) & 0x3F),
            move2_max_pp=MOVE_BASE_PP[GEN_1_MOVES[buffer[0x9]]] + ((int(buffer[0x1E]) >> 6) & 0x03),
            move3=GEN_1_MOVES[buffer[0xA]],
            move3_pp=(int(buffer[0x1F]) & 0x3F),
            move3_max_pp=MOVE_BASE_PP[GEN_1_MOVES[buffer[0xA]]] + ((int(buffer[0x1F]) >> 6) & 0x03),
            move4=GEN_1_MOVES[buffer[0xB]],
            move4_pp=(int(buffer[0x20]) & 0x3F),
            move4_max_pp=MOVE_BASE_PP[GEN_1_MOVES[buffer[0xB]]] + ((int(buffer[0x20]) >> 6) & 0x03),

...
class TestPokemonDisplay(unittest.TestCase):
    def setUp(self):
        self.pokemon = Pokemon.from_memory_buffer("Cap'n", "Green", [
            0x5C, # 0x00 	Index number of the Species 	1 byte 
            0x00, 0x01, #0x01 	Current HP 	2 bytes
            ...
            0x0C, 0x0D, #0x0C 	Original Trainer ID number 	2 bytes
            0x0E, 0x0F, 0x10, #0x0E 	Experience points 	3 bytes
            0x11, 0x12, #0x11 	HP EV data 	2 bytes
            0x13, 0x14, #0x13 	Attack EV data 	2 bytes
            0x15, 0x16, #0x15 	Defense EV data 	2 bytes
            0x17, 0x18, #0x17 	Speed EV data 	2 bytes
            0x19, 0x1A, #0x19 	Special EV data 	2 bytes
            0x1B, 0x1C, #0x1B 	IV data 	2 bytes
            ...
            0x01, 0x02, #0x22 	Maximum HP 	2 bytes
            0x24, 0x25, #0x24 	Attack 	2 bytes
            0x26, 0x27, #0x26 	Defense 	2 bytes
            0x28, 0x29, #0x28 	Speed 	2 bytes
            0x2A, 0x2B #0x2A 	Special 	2 bytes
        ])
    ...
    def test_output(self):
        self.assertEqual(self.pokemon.as_markdown(), f"""
## Cap'n _(Horsea)_
